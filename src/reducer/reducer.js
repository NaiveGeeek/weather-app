...
const weatherReducer = (state=initalStateWeather,action)=>{
   switch (action.type){
       ...
       case actionTypes.GET_WEATHER:{
           return {...state,weather:action.payload.weather,isLoading:false,isError:action.payload.error}
       }
...
const locationReducer = (state=initialStateLocation,action)=>{
    switch(action.type){
        case actionTypes.GET_LOCATION:{
            return {location:action.payload.location,isError:action.payload.error}
        }
